...
#
# Usage:
#   activejob { version = "8.1.2"; }
#   activejob { }  # latest (8.1.2)
#
{
  lib,
  stdenv,
...
if git ? rev then
  gitRevs.${git.rev}
    or (throw "activejob: unknown git rev '${git.rev}'. Available: ${builtins.concatStringsSep ", " (builtins.attrNames gitRevs)}")
else
  versions.${version}
    or (throw "activejob: unknown version '${version}'. Available: ${builtins.concatStringsSep ", " (builtins.attrNames versions)}")
