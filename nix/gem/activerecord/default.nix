...
#
# Usage:
#   activerecord { version = "8.1.2"; }
#   activerecord { }  # latest (8.1.2)
#
{
  lib,
  stdenv,
...
if git ? rev then
  gitRevs.${git.rev}
    or (throw "activerecord: unknown git rev '${git.rev}'. Available: ${builtins.concatStringsSep ", " (builtins.attrNames gitRevs)}")
else
  versions.${version}
    or (throw "activerecord: unknown version '${version}'. Available: ${builtins.concatStringsSep ", " (builtins.attrNames versions)}")
