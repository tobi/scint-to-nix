...
#
# Usage:
#   activemodel { version = "8.1.2"; }
#   activemodel { }  # latest (8.1.2)
#
{
  lib,
  stdenv,
...
if git ? rev then
  gitRevs.${git.rev}
    or (throw "activemodel: unknown git rev '${git.rev}'. Available: ${builtins.concatStringsSep ", " (builtins.attrNames gitRevs)}")
else
  versions.${version}
    or (throw "activemodel: unknown version '${version}'. Available: ${builtins.concatStringsSep ", " (builtins.attrNames versions)}")
