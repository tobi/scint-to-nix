...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/stackprof-0.2.26
    ...
    EOF
        cat > $dest/specifications/stackprof-0.2.26-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "stackprof"
      s.version = "0.2.26"
      s.platform = "$gp"
    ...
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("stackprof", "stackprof", "0.2.26")
    BINSTUB
        chmod +x $dest/bin/stackprof
        cat > $dest/bin/stackprof-flamegraph.pl <<'BINSTUB'
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("stackprof", "stackprof-flamegraph.pl", "0.2.26")
    BINSTUB
        chmod +x $dest/bin/stackprof-flamegraph.pl
        cat > $dest/bin/stackprof-gprof2dot.py <<'BINSTUB'
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("stackprof", "stackprof-gprof2dot.py", "0.2.26")
    BINSTUB
