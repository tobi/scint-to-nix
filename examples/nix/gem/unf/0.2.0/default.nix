...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s unf-0.2.0 $dest/gems/unf-0.2.0-$gp
        ln -s unf-0.2.0 $dest/extensions/${arch}/${rubyVersion}/unf-0.2.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf unf-0.2.0 $dest/gems/unf-0.2.0-universal-darwin
          ln -sf unf-0.2.0 $dest/extensions/${arch}/${rubyVersion}/unf-0.2.0-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/unf-0.2.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "unf"
      s.version = "0.2.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/unf-0.2.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "unf"
      s.version = "0.2.0"
