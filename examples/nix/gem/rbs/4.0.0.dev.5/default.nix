...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf rbs-4.0.0.dev.5 $dest/gems/rbs-4.0.0.dev.5-universal-darwin
          ln -sf rbs-4.0.0.dev.5 $dest/extensions/${arch}/${rubyVersion}/rbs-4.0.0.dev.5-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/rbs-4.0.0.dev.5-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "rbs"
      s.version = "4.0.0.dev.5"
