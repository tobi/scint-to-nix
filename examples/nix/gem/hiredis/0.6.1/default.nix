...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/hiredis-0.6.1
        cp -r . $dest/gems/hiredis-0.6.1/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.1
        mkdir -p $extdir
        ...
        ln -s hiredis-0.6.1 $dest/gems/hiredis-0.6.1-$gp
        ln -s hiredis-0.6.1 $dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf hiredis-0.6.1 $dest/gems/hiredis-0.6.1-universal-darwin
          ln -sf hiredis-0.6.1 $dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.1-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/hiredis-0.6.1.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/hiredis-0.6.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "hiredis"
      s.version = "0.6.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/hiredis-0.6.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "hiredis"
      s.version = "0.6.1"
