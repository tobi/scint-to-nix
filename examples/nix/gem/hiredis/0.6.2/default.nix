...
let
  ...
  overlay = import ../../../../overlays/hiredis.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/hiredis-0.6.2
        cp -r . $dest/gems/hiredis-0.6.2/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.2
        mkdir -p $extdir
        ...
        ln -s hiredis-0.6.2 $dest/gems/hiredis-0.6.2-$gp
        ln -s hiredis-0.6.2 $dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.2-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf hiredis-0.6.2 $dest/gems/hiredis-0.6.2-universal-darwin
          ln -sf hiredis-0.6.2 $dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.2-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/hiredis-0.6.2.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/hiredis-0.6.2-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "hiredis"
      s.version = "0.6.2"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/hiredis-0.6.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "hiredis"
      s.version = "0.6.2"
