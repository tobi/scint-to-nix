...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/hiredis-0.6.3
        cp -r . $dest/gems/hiredis-0.6.3/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.3
        mkdir -p $extdir
        ...
        ln -s hiredis-0.6.3 $dest/gems/hiredis-0.6.3-$gp
        ln -s hiredis-0.6.3 $dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.3-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf hiredis-0.6.3 $dest/gems/hiredis-0.6.3-universal-darwin
          ln -sf hiredis-0.6.3 $dest/extensions/${arch}/${rubyVersion}/hiredis-0.6.3-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/hiredis-0.6.3.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/hiredis-0.6.3-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "hiredis"
      s.version = "0.6.3"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/hiredis-0.6.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "hiredis"
      s.version = "0.6.3"
