...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf rbtrace-0.5.1 $dest/gems/rbtrace-0.5.1-universal-darwin
          ln -sf rbtrace-0.5.1 $dest/extensions/${arch}/${rubyVersion}/rbtrace-0.5.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/rbtrace-0.5.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "rbtrace"
      s.version = "0.5.1"
