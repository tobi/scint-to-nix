...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf fast_blank-0.0.2 $dest/gems/fast_blank-0.0.2-universal-darwin
          ln -sf fast_blank-0.0.2 $dest/extensions/${arch}/${rubyVersion}/fast_blank-0.0.2-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/fast_blank-0.0.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "fast_blank"
      s.version = "0.0.2"
