...
let
  ...
  overlay = import ../../../../overlays/tokenizers.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf tokenizers-0.6.3 $dest/gems/tokenizers-0.6.3-universal-darwin
          ln -sf tokenizers-0.6.3 $dest/extensions/${arch}/${rubyVersion}/tokenizers-0.6.3-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/tokenizers-0.6.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "tokenizers"
      s.version = "0.6.3"
