...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf llhttp-ffi-0.5.0 $dest/gems/llhttp-ffi-0.5.0-universal-darwin
          ln -sf llhttp-ffi-0.5.0 $dest/extensions/${arch}/${rubyVersion}/llhttp-ffi-0.5.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/llhttp-ffi-0.5.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "llhttp-ffi"
      s.version = "0.5.0"
