...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s llhttp-ffi-0.4.0 $dest/gems/llhttp-ffi-0.4.0-$gp
        ln -s llhttp-ffi-0.4.0 $dest/extensions/${arch}/${rubyVersion}/llhttp-ffi-0.4.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf llhttp-ffi-0.4.0 $dest/gems/llhttp-ffi-0.4.0-universal-darwin
          ln -sf llhttp-ffi-0.4.0 $dest/extensions/${arch}/${rubyVersion}/llhttp-ffi-0.4.0-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/llhttp-ffi-0.4.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "llhttp-ffi"
      s.version = "0.4.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/llhttp-ffi-0.4.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "llhttp-ffi"
      s.version = "0.4.0"
