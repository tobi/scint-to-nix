...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/fastlane-2.232.0
        cp -r . $dest/gems/fastlane-2.232.0/
        mkdir -p $dest/specifications
        cat > $dest/specifications/fastlane-2.232.0.gemspec <<'EOF'
    ...
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("fastlane", "bin-proxy", "2.232.0")
    BINSTUB
        chmod +x $dest/bin/bin-proxy
        cat > $dest/bin/console <<'BINSTUB'
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("fastlane", "console", "2.232.0")
    BINSTUB
        chmod +x $dest/bin/console
        cat > $dest/bin/fastlane <<'BINSTUB'
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("fastlane", "fastlane", "2.232.0")
    BINSTUB
        chmod +x $dest/bin/fastlane
        cat > $dest/bin/match_file <<'BINSTUB'
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("fastlane", "match_file", "2.232.0")
    BINSTUB
