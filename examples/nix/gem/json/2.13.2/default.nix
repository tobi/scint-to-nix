...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf json-2.13.2 $dest/gems/json-2.13.2-universal-darwin
          ln -sf json-2.13.2 $dest/extensions/${arch}/${rubyVersion}/json-2.13.2-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/json-2.13.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "json"
      s.version = "2.13.2"
