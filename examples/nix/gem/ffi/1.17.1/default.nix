...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/ffi-1.17.1
        cp -r . $dest/gems/ffi-1.17.1/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/ffi-1.17.1
        mkdir -p $extdir
        ...
        ln -s ffi-1.17.1 $dest/gems/ffi-1.17.1-$gp
        ln -s ffi-1.17.1 $dest/extensions/${arch}/${rubyVersion}/ffi-1.17.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf ffi-1.17.1 $dest/gems/ffi-1.17.1-universal-darwin
          ln -sf ffi-1.17.1 $dest/extensions/${arch}/${rubyVersion}/ffi-1.17.1-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/ffi-1.17.1.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/ffi-1.17.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "ffi"
      s.version = "1.17.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/ffi-1.17.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "ffi"
      s.version = "1.17.1"
