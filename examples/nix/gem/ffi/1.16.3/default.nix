...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf ffi-1.16.3 $dest/gems/ffi-1.16.3-universal-darwin
          ln -sf ffi-1.16.3 $dest/extensions/${arch}/${rubyVersion}/ffi-1.16.3-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/ffi-1.16.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "ffi"
      s.version = "1.16.3"
