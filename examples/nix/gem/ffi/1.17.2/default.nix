...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/ffi-1.17.2
        cp -r . $dest/gems/ffi-1.17.2/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/ffi-1.17.2
        mkdir -p $extdir
        ...
        ln -s ffi-1.17.2 $dest/gems/ffi-1.17.2-$gp
        ln -s ffi-1.17.2 $dest/extensions/${arch}/${rubyVersion}/ffi-1.17.2-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf ffi-1.17.2 $dest/gems/ffi-1.17.2-universal-darwin
          ln -sf ffi-1.17.2 $dest/extensions/${arch}/${rubyVersion}/ffi-1.17.2-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/ffi-1.17.2.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/ffi-1.17.2-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "ffi"
      s.version = "1.17.2"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/ffi-1.17.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "ffi"
      s.version = "1.17.2"
