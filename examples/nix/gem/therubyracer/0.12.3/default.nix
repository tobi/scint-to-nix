...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "therubyracer-0.12.3-source";
  };
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/therubyracer-0.12.3
        cp -r . $dest/gems/therubyracer-0.12.3/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/therubyracer-0.12.3
        mkdir -p $extdir
        ...
        ln -s therubyracer-0.12.3 $dest/gems/therubyracer-0.12.3-$gp
        ln -s therubyracer-0.12.3 $dest/extensions/${arch}/${rubyVersion}/therubyracer-0.12.3-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf therubyracer-0.12.3 $dest/gems/therubyracer-0.12.3-universal-darwin
          ln -sf therubyracer-0.12.3 $dest/extensions/${arch}/${rubyVersion}/therubyracer-0.12.3-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/therubyracer-0.12.3.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/therubyracer-0.12.3-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "therubyracer"
      s.version = "0.12.3"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/therubyracer-0.12.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "therubyracer"
      s.version = "0.12.3"
