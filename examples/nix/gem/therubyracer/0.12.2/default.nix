...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf therubyracer-0.12.2 $dest/gems/therubyracer-0.12.2-universal-darwin
          ln -sf therubyracer-0.12.2 $dest/extensions/${arch}/${rubyVersion}/therubyracer-0.12.2-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/therubyracer-0.12.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "therubyracer"
      s.version = "0.12.2"
