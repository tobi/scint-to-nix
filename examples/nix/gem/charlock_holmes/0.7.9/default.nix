...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf charlock_holmes-0.7.9 $dest/gems/charlock_holmes-0.7.9-universal-darwin
          ln -sf charlock_holmes-0.7.9 $dest/extensions/${arch}/${rubyVersion}/charlock_holmes-0.7.9-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/charlock_holmes-0.7.9-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "charlock_holmes"
      s.version = "0.7.9"
