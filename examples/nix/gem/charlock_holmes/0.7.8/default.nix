...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "charlock_holmes-0.7.8-source";
  };
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/charlock_holmes-0.7.8
        cp -r . $dest/gems/charlock_holmes-0.7.8/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/charlock_holmes-0.7.8
        mkdir -p $extdir
        ...
        ln -s charlock_holmes-0.7.8 $dest/gems/charlock_holmes-0.7.8-$gp
        ln -s charlock_holmes-0.7.8 $dest/extensions/${arch}/${rubyVersion}/charlock_holmes-0.7.8-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf charlock_holmes-0.7.8 $dest/gems/charlock_holmes-0.7.8-universal-darwin
          ln -sf charlock_holmes-0.7.8 $dest/extensions/${arch}/${rubyVersion}/charlock_holmes-0.7.8-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/charlock_holmes-0.7.8.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/charlock_holmes-0.7.8-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "charlock_holmes"
      s.version = "0.7.8"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/charlock_holmes-0.7.8-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "charlock_holmes"
      s.version = "0.7.8"
