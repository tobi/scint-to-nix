...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf sassc-2.4.0 $dest/gems/sassc-2.4.0-universal-darwin
          ln -sf sassc-2.4.0 $dest/extensions/${arch}/${rubyVersion}/sassc-2.4.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/sassc-2.4.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "sassc"
      s.version = "2.4.0"
