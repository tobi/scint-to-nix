...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf cgi-0.3.6 $dest/gems/cgi-0.3.6-universal-darwin
          ln -sf cgi-0.3.6 $dest/extensions/${arch}/${rubyVersion}/cgi-0.3.6-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/cgi-0.3.6-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "cgi"
      s.version = "0.3.6"
