...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf redcarpet-3.6.0 $dest/gems/redcarpet-3.6.0-universal-darwin
          ln -sf redcarpet-3.6.0 $dest/extensions/${arch}/${rubyVersion}/redcarpet-3.6.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/redcarpet-3.6.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "redcarpet"
      s.version = "3.6.0"
