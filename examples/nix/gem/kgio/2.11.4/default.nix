...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf kgio-2.11.4 $dest/gems/kgio-2.11.4-universal-darwin
          ln -sf kgio-2.11.4 $dest/extensions/${arch}/${rubyVersion}/kgio-2.11.4-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/kgio-2.11.4-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "kgio"
      s.version = "2.11.4"
