...
let
  ...
  overlay = import ../../../../overlays/datadog.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf datadog-2.19.0 $dest/gems/datadog-2.19.0-universal-darwin
          ln -sf datadog-2.19.0 $dest/extensions/${arch}/${rubyVersion}/datadog-2.19.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/datadog-2.19.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "datadog"
      s.version = "2.19.0"
