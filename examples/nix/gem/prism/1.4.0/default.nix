...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf prism-1.4.0 $dest/gems/prism-1.4.0-universal-darwin
          ln -sf prism-1.4.0 $dest/extensions/${arch}/${rubyVersion}/prism-1.4.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/prism-1.4.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "prism"
      s.version = "1.4.0"
