...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf ddtrace-1.23.3 $dest/gems/ddtrace-1.23.3-universal-darwin
          ln -sf ddtrace-1.23.3 $dest/extensions/${arch}/${rubyVersion}/ddtrace-1.23.3-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/ddtrace-1.23.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "ddtrace"
      s.version = "1.23.3"
