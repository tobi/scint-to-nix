...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/rugged-1.9.0
        cp -r . $dest/gems/rugged-1.9.0/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/rugged-1.9.0
        mkdir -p $extdir
        ...
        ln -s rugged-1.9.0 $dest/gems/rugged-1.9.0-$gp
        ln -s rugged-1.9.0 $dest/extensions/${arch}/${rubyVersion}/rugged-1.9.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf rugged-1.9.0 $dest/gems/rugged-1.9.0-universal-darwin
          ln -sf rugged-1.9.0 $dest/extensions/${arch}/${rubyVersion}/rugged-1.9.0-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/rugged-1.9.0.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/rugged-1.9.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "rugged"
      s.version = "1.9.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/rugged-1.9.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "rugged"
      s.version = "1.9.0"
