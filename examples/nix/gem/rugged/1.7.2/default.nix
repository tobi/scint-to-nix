...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/rugged-1.7.2
        cp -r . $dest/gems/rugged-1.7.2/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/rugged-1.7.2
        mkdir -p $extdir
        ...
        ln -s rugged-1.7.2 $dest/gems/rugged-1.7.2-$gp
        ln -s rugged-1.7.2 $dest/extensions/${arch}/${rubyVersion}/rugged-1.7.2-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf rugged-1.7.2 $dest/gems/rugged-1.7.2-universal-darwin
          ln -sf rugged-1.7.2 $dest/extensions/${arch}/${rubyVersion}/rugged-1.7.2-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/rugged-1.7.2.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/rugged-1.7.2-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "rugged"
      s.version = "1.7.2"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/rugged-1.7.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "rugged"
      s.version = "1.7.2"
