...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf kostya-sigar-2.0.11 $dest/gems/kostya-sigar-2.0.11-universal-darwin
          ln -sf kostya-sigar-2.0.11 $dest/extensions/${arch}/${rubyVersion}/kostya-sigar-2.0.11-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/kostya-sigar-2.0.11-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "kostya-sigar"
      s.version = "2.0.11"
