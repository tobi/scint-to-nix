...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "websocket-driver-0.8.0-source";
  };
  ...
  installPhase = ''
        ...
        ln -s websocket-driver-0.8.0 $dest/gems/websocket-driver-0.8.0-$gp
        ln -s websocket-driver-0.8.0 $dest/extensions/${arch}/${rubyVersion}/websocket-driver-0.8.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf websocket-driver-0.8.0 $dest/gems/websocket-driver-0.8.0-universal-darwin
          ln -sf websocket-driver-0.8.0 $dest/extensions/${arch}/${rubyVersion}/websocket-driver-0.8.0-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/websocket-driver-0.8.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "websocket-driver"
      s.version = "0.8.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/websocket-driver-0.8.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "websocket-driver"
      s.version = "0.8.0"
