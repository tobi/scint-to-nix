...
let
  ...
  overlay = import ../../../../overlays/openssl.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/openssl-3.2.0
        cp -r . $dest/gems/openssl-3.2.0/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/openssl-3.2.0
        mkdir -p $extdir
        ...
        ln -s openssl-3.2.0 $dest/gems/openssl-3.2.0-$gp
        ln -s openssl-3.2.0 $dest/extensions/${arch}/${rubyVersion}/openssl-3.2.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf openssl-3.2.0 $dest/gems/openssl-3.2.0-universal-darwin
          ln -sf openssl-3.2.0 $dest/extensions/${arch}/${rubyVersion}/openssl-3.2.0-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/openssl-3.2.0.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/openssl-3.2.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "openssl"
      s.version = "3.2.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/openssl-3.2.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "openssl"
      s.version = "3.2.0"
