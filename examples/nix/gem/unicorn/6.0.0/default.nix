...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf unicorn-6.0.0 $dest/gems/unicorn-6.0.0-universal-darwin
          ln -sf unicorn-6.0.0 $dest/extensions/${arch}/${rubyVersion}/unicorn-6.0.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/unicorn-6.0.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "unicorn"
      s.version = "6.0.0"
