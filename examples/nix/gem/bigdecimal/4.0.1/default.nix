...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf bigdecimal-4.0.1 $dest/gems/bigdecimal-4.0.1-universal-darwin
          ln -sf bigdecimal-4.0.1 $dest/extensions/${arch}/${rubyVersion}/bigdecimal-4.0.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/bigdecimal-4.0.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "bigdecimal"
      s.version = "4.0.1"
