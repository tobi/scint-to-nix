...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf commonmarker-0.23.10 $dest/gems/commonmarker-0.23.10-universal-darwin
          ln -sf commonmarker-0.23.10 $dest/extensions/${arch}/${rubyVersion}/commonmarker-0.23.10-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/commonmarker-0.23.10-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "commonmarker"
      s.version = "0.23.10"
