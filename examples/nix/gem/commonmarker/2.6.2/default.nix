...
let
  ...
  overlay = import ../../../../overlays/commonmarker.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "commonmarker-2.6.2-source";
  };
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/commonmarker-2.6.2
        cp -r . $dest/gems/commonmarker-2.6.2/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/commonmarker-2.6.2
        mkdir -p $extdir
        ...
        ln -s commonmarker-2.6.2 $dest/gems/commonmarker-2.6.2-$gp
        ln -s commonmarker-2.6.2 $dest/extensions/${arch}/${rubyVersion}/commonmarker-2.6.2-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf commonmarker-2.6.2 $dest/gems/commonmarker-2.6.2-universal-darwin
          ln -sf commonmarker-2.6.2 $dest/extensions/${arch}/${rubyVersion}/commonmarker-2.6.2-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/commonmarker-2.6.2.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/commonmarker-2.6.2-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "commonmarker"
      s.version = "2.6.2"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/commonmarker-2.6.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "commonmarker"
      s.version = "2.6.2"
