...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf commonmarker-2.6.3 $dest/gems/commonmarker-2.6.3-universal-darwin
          ln -sf commonmarker-2.6.3 $dest/extensions/${arch}/${rubyVersion}/commonmarker-2.6.3-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/commonmarker-2.6.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "commonmarker"
      s.version = "2.6.3"
