...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "commonmarker-2.6.1-source";
  };
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/commonmarker-2.6.1
        cp -r . $dest/gems/commonmarker-2.6.1/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/commonmarker-2.6.1
        mkdir -p $extdir
        ...
        ln -s commonmarker-2.6.1 $dest/gems/commonmarker-2.6.1-$gp
        ln -s commonmarker-2.6.1 $dest/extensions/${arch}/${rubyVersion}/commonmarker-2.6.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf commonmarker-2.6.1 $dest/gems/commonmarker-2.6.1-universal-darwin
          ln -sf commonmarker-2.6.1 $dest/extensions/${arch}/${rubyVersion}/commonmarker-2.6.1-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/commonmarker-2.6.1.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/commonmarker-2.6.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "commonmarker"
      s.version = "2.6.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/commonmarker-2.6.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "commonmarker"
      s.version = "2.6.1"
