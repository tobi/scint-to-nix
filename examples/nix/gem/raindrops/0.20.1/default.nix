...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf raindrops-0.20.1 $dest/gems/raindrops-0.20.1-universal-darwin
          ln -sf raindrops-0.20.1 $dest/extensions/${arch}/${rubyVersion}/raindrops-0.20.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/raindrops-0.20.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "raindrops"
      s.version = "0.20.1"
