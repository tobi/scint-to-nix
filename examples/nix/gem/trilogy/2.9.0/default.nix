...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf trilogy-2.9.0 $dest/gems/trilogy-2.9.0-universal-darwin
          ln -sf trilogy-2.9.0 $dest/extensions/${arch}/${rubyVersion}/trilogy-2.9.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/trilogy-2.9.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "trilogy"
      s.version = "2.9.0"
