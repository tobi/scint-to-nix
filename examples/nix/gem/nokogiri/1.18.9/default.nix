...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf nokogiri-1.18.9 $dest/gems/nokogiri-1.18.9-universal-darwin
          ln -sf nokogiri-1.18.9 $dest/extensions/${arch}/${rubyVersion}/nokogiri-1.18.9-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/nokogiri-1.18.9-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "nokogiri"
      s.version = "1.18.9"
    ...
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("nokogiri", "nokogiri", "1.18.9")
    BINSTUB
