...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/nokogiri-1.16.5
        cp -r . $dest/gems/nokogiri-1.16.5/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/nokogiri-1.16.5
        mkdir -p $extdir
        ...
        ln -s nokogiri-1.16.5 $dest/gems/nokogiri-1.16.5-$gp
        ln -s nokogiri-1.16.5 $dest/extensions/${arch}/${rubyVersion}/nokogiri-1.16.5-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf nokogiri-1.16.5 $dest/gems/nokogiri-1.16.5-universal-darwin
          ln -sf nokogiri-1.16.5 $dest/extensions/${arch}/${rubyVersion}/nokogiri-1.16.5-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/nokogiri-1.16.5.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/nokogiri-1.16.5-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "nokogiri"
      s.version = "1.16.5"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/nokogiri-1.16.5-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "nokogiri"
      s.version = "1.16.5"
    ...
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("nokogiri", "nokogiri", "1.16.5")
    BINSTUB
