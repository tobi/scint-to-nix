...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf jaro_winkler-1.5.6 $dest/gems/jaro_winkler-1.5.6-universal-darwin
          ln -sf jaro_winkler-1.5.6 $dest/extensions/${arch}/${rubyVersion}/jaro_winkler-1.5.6-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/jaro_winkler-1.5.6-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "jaro_winkler"
      s.version = "1.5.6"
