...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf bootsnap-1.21.0 $dest/gems/bootsnap-1.21.0-universal-darwin
          ln -sf bootsnap-1.21.0 $dest/extensions/${arch}/${rubyVersion}/bootsnap-1.21.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/bootsnap-1.21.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "bootsnap"
      s.version = "1.21.0"
