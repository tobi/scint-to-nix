...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s bootsnap-1.19.0 $dest/gems/bootsnap-1.19.0-$gp
        ln -s bootsnap-1.19.0 $dest/extensions/${arch}/${rubyVersion}/bootsnap-1.19.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf bootsnap-1.19.0 $dest/gems/bootsnap-1.19.0-universal-darwin
          ln -sf bootsnap-1.19.0 $dest/extensions/${arch}/${rubyVersion}/bootsnap-1.19.0-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/bootsnap-1.19.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "bootsnap"
      s.version = "1.19.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/bootsnap-1.19.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "bootsnap"
      s.version = "1.19.0"
