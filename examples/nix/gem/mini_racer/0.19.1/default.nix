...
let
  ...
  overlay = import ../../../../overlays/mini_racer.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf mini_racer-0.19.1 $dest/gems/mini_racer-0.19.1-universal-darwin
          ln -sf mini_racer-0.19.1 $dest/extensions/${arch}/${rubyVersion}/mini_racer-0.19.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/mini_racer-0.19.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "mini_racer"
      s.version = "0.19.1"
