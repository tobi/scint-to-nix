...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/pg-1.6.2
        cp -r . $dest/gems/pg-1.6.2/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/pg-1.6.2
        mkdir -p $extdir
        ...
        ln -s pg-1.6.2 $dest/gems/pg-1.6.2-$gp
        ln -s pg-1.6.2 $dest/extensions/${arch}/${rubyVersion}/pg-1.6.2-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf pg-1.6.2 $dest/gems/pg-1.6.2-universal-darwin
          ln -sf pg-1.6.2 $dest/extensions/${arch}/${rubyVersion}/pg-1.6.2-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/pg-1.6.2.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/pg-1.6.2-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "pg"
      s.version = "1.6.2"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/pg-1.6.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "pg"
      s.version = "1.6.2"
