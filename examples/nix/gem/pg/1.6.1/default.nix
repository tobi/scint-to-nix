...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/pg-1.6.1
        cp -r . $dest/gems/pg-1.6.1/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/pg-1.6.1
        mkdir -p $extdir
        ...
        ln -s pg-1.6.1 $dest/gems/pg-1.6.1-$gp
        ln -s pg-1.6.1 $dest/extensions/${arch}/${rubyVersion}/pg-1.6.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf pg-1.6.1 $dest/gems/pg-1.6.1-universal-darwin
          ln -sf pg-1.6.1 $dest/extensions/${arch}/${rubyVersion}/pg-1.6.1-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/pg-1.6.1.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/pg-1.6.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "pg"
      s.version = "1.6.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/pg-1.6.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "pg"
      s.version = "1.6.1"
