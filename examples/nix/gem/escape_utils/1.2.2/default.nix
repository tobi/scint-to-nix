...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf escape_utils-1.2.2 $dest/gems/escape_utils-1.2.2-universal-darwin
          ln -sf escape_utils-1.2.2 $dest/extensions/${arch}/${rubyVersion}/escape_utils-1.2.2-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/escape_utils-1.2.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "escape_utils"
      s.version = "1.2.2"
