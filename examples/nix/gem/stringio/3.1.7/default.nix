...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf stringio-3.1.7 $dest/gems/stringio-3.1.7-universal-darwin
          ln -sf stringio-3.1.7 $dest/extensions/${arch}/${rubyVersion}/stringio-3.1.7-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/stringio-3.1.7-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "stringio"
      s.version = "3.1.7"
