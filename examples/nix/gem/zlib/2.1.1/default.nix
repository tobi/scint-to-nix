...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf zlib-2.1.1 $dest/gems/zlib-2.1.1-universal-darwin
          ln -sf zlib-2.1.1 $dest/extensions/${arch}/${rubyVersion}/zlib-2.1.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/zlib-2.1.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "zlib"
      s.version = "2.1.1"
