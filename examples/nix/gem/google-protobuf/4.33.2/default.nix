...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "google-protobuf-4.33.2-source";
  };
  ...
  installPhase = ''
        ...
        ln -s google-protobuf-4.33.2 $dest/gems/google-protobuf-4.33.2-$gp
        ln -s google-protobuf-4.33.2 $dest/extensions/${arch}/${rubyVersion}/google-protobuf-4.33.2-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf google-protobuf-4.33.2 $dest/gems/google-protobuf-4.33.2-universal-darwin
          ln -sf google-protobuf-4.33.2 $dest/extensions/${arch}/${rubyVersion}/google-protobuf-4.33.2-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/google-protobuf-4.33.2-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "google-protobuf"
      s.version = "4.33.2"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/google-protobuf-4.33.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "google-protobuf"
      s.version = "4.33.2"
