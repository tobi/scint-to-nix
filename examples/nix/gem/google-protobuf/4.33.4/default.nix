...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "google-protobuf-4.33.4-source";
  };
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/google-protobuf-4.33.4
        cp -r . $dest/gems/google-protobuf-4.33.4/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/google-protobuf-4.33.4
        mkdir -p $extdir
        ...
        ln -s google-protobuf-4.33.4 $dest/gems/google-protobuf-4.33.4-$gp
        ln -s google-protobuf-4.33.4 $dest/extensions/${arch}/${rubyVersion}/google-protobuf-4.33.4-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf google-protobuf-4.33.4 $dest/gems/google-protobuf-4.33.4-universal-darwin
          ln -sf google-protobuf-4.33.4 $dest/extensions/${arch}/${rubyVersion}/google-protobuf-4.33.4-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/google-protobuf-4.33.4.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/google-protobuf-4.33.4-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "google-protobuf"
      s.version = "4.33.4"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/google-protobuf-4.33.4-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "google-protobuf"
      s.version = "4.33.4"
