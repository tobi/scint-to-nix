...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "google-protobuf-4.33.5-source";
  };
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/google-protobuf-4.33.5
        cp -r . $dest/gems/google-protobuf-4.33.5/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/google-protobuf-4.33.5
        mkdir -p $extdir
        ...
        ln -s google-protobuf-4.33.5 $dest/gems/google-protobuf-4.33.5-$gp
        ln -s google-protobuf-4.33.5 $dest/extensions/${arch}/${rubyVersion}/google-protobuf-4.33.5-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf google-protobuf-4.33.5 $dest/gems/google-protobuf-4.33.5-universal-darwin
          ln -sf google-protobuf-4.33.5 $dest/extensions/${arch}/${rubyVersion}/google-protobuf-4.33.5-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/google-protobuf-4.33.5.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/google-protobuf-4.33.5-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "google-protobuf"
      s.version = "4.33.5"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/google-protobuf-4.33.5-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "google-protobuf"
      s.version = "4.33.5"
