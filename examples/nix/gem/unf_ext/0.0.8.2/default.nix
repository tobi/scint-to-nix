...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf unf_ext-0.0.8.2 $dest/gems/unf_ext-0.0.8.2-universal-darwin
          ln -sf unf_ext-0.0.8.2 $dest/extensions/${arch}/${rubyVersion}/unf_ext-0.0.8.2-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/unf_ext-0.0.8.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "unf_ext"
      s.version = "0.0.8.2"
