...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s puma-6.4.3 $dest/gems/puma-6.4.3-$gp
        ln -s puma-6.4.3 $dest/extensions/${arch}/${rubyVersion}/puma-6.4.3-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf puma-6.4.3 $dest/gems/puma-6.4.3-universal-darwin
          ln -sf puma-6.4.3 $dest/extensions/${arch}/${rubyVersion}/puma-6.4.3-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/puma-6.4.3-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "puma"
      s.version = "6.4.3"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/puma-6.4.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "puma"
      s.version = "6.4.3"
