...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/puma-6.5.0
        cp -r . $dest/gems/puma-6.5.0/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/puma-6.5.0
        mkdir -p $extdir
        ...
        ln -s puma-6.5.0 $dest/gems/puma-6.5.0-$gp
        ln -s puma-6.5.0 $dest/extensions/${arch}/${rubyVersion}/puma-6.5.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf puma-6.5.0 $dest/gems/puma-6.5.0-universal-darwin
          ln -sf puma-6.5.0 $dest/extensions/${arch}/${rubyVersion}/puma-6.5.0-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/puma-6.5.0.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/puma-6.5.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "puma"
      s.version = "6.5.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/puma-6.5.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "puma"
      s.version = "6.5.0"
    ...
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("puma", "puma", "6.5.0")
    BINSTUB
        chmod +x $dest/bin/puma
        cat > $dest/bin/pumactl <<'BINSTUB'
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("puma", "pumactl", "6.5.0")
    BINSTUB
