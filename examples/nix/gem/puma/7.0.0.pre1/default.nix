...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/puma-7.0.0.pre1
        cp -r . $dest/gems/puma-7.0.0.pre1/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/puma-7.0.0.pre1
        mkdir -p $extdir
        ...
        ln -s puma-7.0.0.pre1 $dest/gems/puma-7.0.0.pre1-$gp
        ln -s puma-7.0.0.pre1 $dest/extensions/${arch}/${rubyVersion}/puma-7.0.0.pre1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf puma-7.0.0.pre1 $dest/gems/puma-7.0.0.pre1-universal-darwin
          ln -sf puma-7.0.0.pre1 $dest/extensions/${arch}/${rubyVersion}/puma-7.0.0.pre1-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/puma-7.0.0.pre1.gemspec <<'EOF'
    Gem::Specification.new do |s|
      s.name = "puma"
      s.version = "7.0.0.pre1"
      s.summary = "puma"
      s.require_paths = ["lib"]
      s.bindir = "bin"
    ...
    EOF
        cat > $dest/specifications/puma-7.0.0.pre1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "puma"
      s.version = "7.0.0.pre1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/puma-7.0.0.pre1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "puma"
      s.version = "7.0.0.pre1"
    ...
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("puma", "puma", "7.0.0.pre1")
    BINSTUB
        chmod +x $dest/bin/puma
        cat > $dest/bin/pumactl <<'BINSTUB'
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("puma", "pumactl", "7.0.0.pre1")
    BINSTUB
