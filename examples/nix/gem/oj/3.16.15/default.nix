...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf oj-3.16.15 $dest/gems/oj-3.16.15-universal-darwin
          ln -sf oj-3.16.15 $dest/extensions/${arch}/${rubyVersion}/oj-3.16.15-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/oj-3.16.15-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "oj"
      s.version = "3.16.15"
