...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s mittens-0.3.1 $dest/gems/mittens-0.3.1-$gp
        ln -s mittens-0.3.1 $dest/extensions/${arch}/${rubyVersion}/mittens-0.3.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf mittens-0.3.1 $dest/gems/mittens-0.3.1-universal-darwin
          ln -sf mittens-0.3.1 $dest/extensions/${arch}/${rubyVersion}/mittens-0.3.1-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/mittens-0.3.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "mittens"
      s.version = "0.3.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/mittens-0.3.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "mittens"
      s.version = "0.3.1"
