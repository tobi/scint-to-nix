...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/rmagick-6.1.4
        cp -r . $dest/gems/rmagick-6.1.4/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/rmagick-6.1.4
        mkdir -p $extdir
        ...
        ln -s rmagick-6.1.4 $dest/gems/rmagick-6.1.4-$gp
        ln -s rmagick-6.1.4 $dest/extensions/${arch}/${rubyVersion}/rmagick-6.1.4-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf rmagick-6.1.4 $dest/gems/rmagick-6.1.4-universal-darwin
          ln -sf rmagick-6.1.4 $dest/extensions/${arch}/${rubyVersion}/rmagick-6.1.4-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/rmagick-6.1.4.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/rmagick-6.1.4-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "rmagick"
      s.version = "6.1.4"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/rmagick-6.1.4-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "rmagick"
      s.version = "6.1.4"
