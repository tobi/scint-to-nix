...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf tiktoken_ruby-0.0.15.1 $dest/gems/tiktoken_ruby-0.0.15.1-universal-darwin
          ln -sf tiktoken_ruby-0.0.15.1 $dest/extensions/${arch}/${rubyVersion}/tiktoken_ruby-0.0.15.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/tiktoken_ruby-0.0.15.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "tiktoken_ruby"
      s.version = "0.0.15.1"
