...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s sqlite3-2.8.1 $dest/gems/sqlite3-2.8.1-$gp
        ln -s sqlite3-2.8.1 $dest/extensions/${arch}/${rubyVersion}/sqlite3-2.8.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf sqlite3-2.8.1 $dest/gems/sqlite3-2.8.1-universal-darwin
          ln -sf sqlite3-2.8.1 $dest/extensions/${arch}/${rubyVersion}/sqlite3-2.8.1-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/sqlite3-2.8.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "sqlite3"
      s.version = "2.8.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/sqlite3-2.8.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "sqlite3"
      s.version = "2.8.1"
