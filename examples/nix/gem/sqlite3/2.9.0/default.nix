...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s sqlite3-2.9.0 $dest/gems/sqlite3-2.9.0-$gp
        ln -s sqlite3-2.9.0 $dest/extensions/${arch}/${rubyVersion}/sqlite3-2.9.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf sqlite3-2.9.0 $dest/gems/sqlite3-2.9.0-universal-darwin
          ln -sf sqlite3-2.9.0 $dest/extensions/${arch}/${rubyVersion}/sqlite3-2.9.0-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/sqlite3-2.9.0.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/sqlite3-2.9.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "sqlite3"
      s.version = "2.9.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/sqlite3-2.9.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "sqlite3"
      s.version = "2.9.0"
