...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf cppjieba_rb-0.4.4 $dest/gems/cppjieba_rb-0.4.4-universal-darwin
          ln -sf cppjieba_rb-0.4.4 $dest/extensions/${arch}/${rubyVersion}/cppjieba_rb-0.4.4-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/cppjieba_rb-0.4.4-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "cppjieba_rb"
      s.version = "0.4.4"
