...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf debug-1.9.2 $dest/gems/debug-1.9.2-universal-darwin
          ln -sf debug-1.9.2 $dest/extensions/${arch}/${rubyVersion}/debug-1.9.2-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/debug-1.9.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "debug"
      s.version = "1.9.2"
