...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s psych-5.2.6 $dest/gems/psych-5.2.6-$gp
        ln -s psych-5.2.6 $dest/extensions/${arch}/${rubyVersion}/psych-5.2.6-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf psych-5.2.6 $dest/gems/psych-5.2.6-universal-darwin
          ln -sf psych-5.2.6 $dest/extensions/${arch}/${rubyVersion}/psych-5.2.6-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/psych-5.2.6-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "psych"
      s.version = "5.2.6"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/psych-5.2.6-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "psych"
      s.version = "5.2.6"
