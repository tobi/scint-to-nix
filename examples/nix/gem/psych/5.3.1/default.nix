...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/psych-5.3.1
        cp -r . $dest/gems/psych-5.3.1/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/psych-5.3.1
        mkdir -p $extdir
        ...
        ln -s psych-5.3.1 $dest/gems/psych-5.3.1-$gp
        ln -s psych-5.3.1 $dest/extensions/${arch}/${rubyVersion}/psych-5.3.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf psych-5.3.1 $dest/gems/psych-5.3.1-universal-darwin
          ln -sf psych-5.3.1 $dest/extensions/${arch}/${rubyVersion}/psych-5.3.1-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/psych-5.3.1.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/psych-5.3.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "psych"
      s.version = "5.3.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/psych-5.3.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "psych"
      s.version = "5.3.1"
