...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/psych-5.1.2
        cp -r . $dest/gems/psych-5.1.2/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/psych-5.1.2
        mkdir -p $extdir
        ...
        ln -s psych-5.1.2 $dest/gems/psych-5.1.2-$gp
        ln -s psych-5.1.2 $dest/extensions/${arch}/${rubyVersion}/psych-5.1.2-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf psych-5.1.2 $dest/gems/psych-5.1.2-universal-darwin
          ln -sf psych-5.1.2 $dest/extensions/${arch}/${rubyVersion}/psych-5.1.2-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/psych-5.1.2.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/psych-5.1.2-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "psych"
      s.version = "5.1.2"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/psych-5.1.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "psych"
      s.version = "5.1.2"
