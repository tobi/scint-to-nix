...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/mysql2-0.5.7
        cp -r . $dest/gems/mysql2-0.5.7/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.7
        mkdir -p $extdir
        ...
        ln -s mysql2-0.5.7 $dest/gems/mysql2-0.5.7-$gp
        ln -s mysql2-0.5.7 $dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.7-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf mysql2-0.5.7 $dest/gems/mysql2-0.5.7-universal-darwin
          ln -sf mysql2-0.5.7 $dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.7-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/mysql2-0.5.7.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/mysql2-0.5.7-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "mysql2"
      s.version = "0.5.7"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/mysql2-0.5.7-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "mysql2"
      s.version = "0.5.7"
