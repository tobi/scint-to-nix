...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/mysql2-0.5.6
        cp -r . $dest/gems/mysql2-0.5.6/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.6
        mkdir -p $extdir
        ...
        ln -s mysql2-0.5.6 $dest/gems/mysql2-0.5.6-$gp
        ln -s mysql2-0.5.6 $dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.6-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf mysql2-0.5.6 $dest/gems/mysql2-0.5.6-universal-darwin
          ln -sf mysql2-0.5.6 $dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.6-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/mysql2-0.5.6.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/mysql2-0.5.6-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "mysql2"
      s.version = "0.5.6"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/mysql2-0.5.6-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "mysql2"
      s.version = "0.5.6"
