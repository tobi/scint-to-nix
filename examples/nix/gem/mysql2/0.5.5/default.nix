...
let
  ...
  overlay = import ../../../../overlays/mysql2.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/mysql2-0.5.5
        cp -r . $dest/gems/mysql2-0.5.5/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.5
        mkdir -p $extdir
        ...
        ln -s mysql2-0.5.5 $dest/gems/mysql2-0.5.5-$gp
        ln -s mysql2-0.5.5 $dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.5-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf mysql2-0.5.5 $dest/gems/mysql2-0.5.5-universal-darwin
          ln -sf mysql2-0.5.5 $dest/extensions/${arch}/${rubyVersion}/mysql2-0.5.5-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/mysql2-0.5.5.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/mysql2-0.5.5-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "mysql2"
      s.version = "0.5.5"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/mysql2-0.5.5-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "mysql2"
      s.version = "0.5.5"
