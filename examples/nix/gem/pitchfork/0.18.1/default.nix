...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s pitchfork-0.18.1 $dest/gems/pitchfork-0.18.1-$gp
        ln -s pitchfork-0.18.1 $dest/extensions/${arch}/${rubyVersion}/pitchfork-0.18.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf pitchfork-0.18.1 $dest/gems/pitchfork-0.18.1-universal-darwin
          ln -sf pitchfork-0.18.1 $dest/extensions/${arch}/${rubyVersion}/pitchfork-0.18.1-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/pitchfork-0.18.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "pitchfork"
      s.version = "0.18.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/pitchfork-0.18.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "pitchfork"
      s.version = "0.18.1"
