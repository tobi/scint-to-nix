...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf bcrypt_pbkdf-1.1.2 $dest/gems/bcrypt_pbkdf-1.1.2-universal-darwin
          ln -sf bcrypt_pbkdf-1.1.2 $dest/extensions/${arch}/${rubyVersion}/bcrypt_pbkdf-1.1.2-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/bcrypt_pbkdf-1.1.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "bcrypt_pbkdf"
      s.version = "1.1.2"
