...
let
  ...
  overlay = import ../../../../overlays/extralite-bundle.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf extralite-bundle-2.13 $dest/gems/extralite-bundle-2.13-universal-darwin
          ln -sf extralite-bundle-2.13 $dest/extensions/${arch}/${rubyVersion}/extralite-bundle-2.13-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/extralite-bundle-2.13-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "extralite-bundle"
      s.version = "2.13"
