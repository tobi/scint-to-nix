...
let
  ...
  overlay = import ../../../../overlays/libv8.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "libv8-7.3.492.27.1-source";
  };
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/libv8-7.3.492.27.1
        cp -r . $dest/gems/libv8-7.3.492.27.1/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/libv8-7.3.492.27.1
        mkdir -p $extdir
        ...
        ln -s libv8-7.3.492.27.1 $dest/gems/libv8-7.3.492.27.1-$gp
        ln -s libv8-7.3.492.27.1 $dest/extensions/${arch}/${rubyVersion}/libv8-7.3.492.27.1-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf libv8-7.3.492.27.1 $dest/gems/libv8-7.3.492.27.1-universal-darwin
          ln -sf libv8-7.3.492.27.1 $dest/extensions/${arch}/${rubyVersion}/libv8-7.3.492.27.1-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/libv8-7.3.492.27.1.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/libv8-7.3.492.27.1-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "libv8"
      s.version = "7.3.492.27.1"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/libv8-7.3.492.27.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "libv8"
      s.version = "7.3.492.27.1"
