...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf libv8-8.4.255.0 $dest/gems/libv8-8.4.255.0-universal-darwin
          ln -sf libv8-8.4.255.0 $dest/extensions/${arch}/${rubyVersion}/libv8-8.4.255.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/libv8-8.4.255.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "libv8"
      s.version = "8.4.255.0"
