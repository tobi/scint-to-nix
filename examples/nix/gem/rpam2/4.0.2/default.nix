...
let
  ...
  overlay = import ../../../../overlays/rpam2.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/rpam2-4.0.2
        cp -r . $dest/gems/rpam2-4.0.2/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/rpam2-4.0.2
        mkdir -p $extdir
        ...
        ln -s rpam2-4.0.2 $dest/gems/rpam2-4.0.2-$gp
        ln -s rpam2-4.0.2 $dest/extensions/${arch}/${rubyVersion}/rpam2-4.0.2-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf rpam2-4.0.2 $dest/gems/rpam2-4.0.2-universal-darwin
          ln -sf rpam2-4.0.2 $dest/extensions/${arch}/${rubyVersion}/rpam2-4.0.2-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/rpam2-4.0.2.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/rpam2-4.0.2-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "rpam2"
      s.version = "4.0.2"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/rpam2-4.0.2-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "rpam2"
      s.version = "4.0.2"
