...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf stackprof-0.2.25 $dest/gems/stackprof-0.2.25-universal-darwin
          ln -sf stackprof-0.2.25 $dest/extensions/${arch}/${rubyVersion}/stackprof-0.2.25-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/stackprof-0.2.25-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "stackprof"
      s.version = "0.2.25"
