...
let
  ...
  overlay = import ../../../../overlays/gpgme.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/gpgme-2.0.25
        cp -r . $dest/gems/gpgme-2.0.25/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/gpgme-2.0.25
        mkdir -p $extdir
        ...
        ln -s gpgme-2.0.25 $dest/gems/gpgme-2.0.25-$gp
        ln -s gpgme-2.0.25 $dest/extensions/${arch}/${rubyVersion}/gpgme-2.0.25-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf gpgme-2.0.25 $dest/gems/gpgme-2.0.25-universal-darwin
          ln -sf gpgme-2.0.25 $dest/extensions/${arch}/${rubyVersion}/gpgme-2.0.25-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/gpgme-2.0.25.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/gpgme-2.0.25-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "gpgme"
      s.version = "2.0.25"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/gpgme-2.0.25-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "gpgme"
      s.version = "2.0.25"
