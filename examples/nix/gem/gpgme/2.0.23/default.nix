...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf gpgme-2.0.23 $dest/gems/gpgme-2.0.23-universal-darwin
          ln -sf gpgme-2.0.23 $dest/extensions/${arch}/${rubyVersion}/gpgme-2.0.23-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/gpgme-2.0.23-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "gpgme"
      s.version = "2.0.23"
