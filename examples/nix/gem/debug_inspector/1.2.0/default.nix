...
stdenv.mkDerivation {
  ...
  src = builtins.path {
    path = ./source;
    name = "debug_inspector-1.2.0-source";
  };
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf debug_inspector-1.2.0 $dest/gems/debug_inspector-1.2.0-universal-darwin
          ln -sf debug_inspector-1.2.0 $dest/extensions/${arch}/${rubyVersion}/debug_inspector-1.2.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/debug_inspector-1.2.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "debug_inspector"
      s.version = "1.2.0"
