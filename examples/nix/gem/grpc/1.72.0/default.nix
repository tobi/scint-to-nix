...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf grpc-1.72.0 $dest/gems/grpc-1.72.0-universal-darwin
          ln -sf grpc-1.72.0 $dest/extensions/${arch}/${rubyVersion}/grpc-1.72.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/grpc-1.72.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "grpc"
      s.version = "1.72.0"
