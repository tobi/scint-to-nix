...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf ffi-yajl-2.7.6 $dest/gems/ffi-yajl-2.7.6-universal-darwin
          ln -sf ffi-yajl-2.7.6 $dest/extensions/${arch}/${rubyVersion}/ffi-yajl-2.7.6-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/ffi-yajl-2.7.6-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "ffi-yajl"
      s.version = "2.7.6"
