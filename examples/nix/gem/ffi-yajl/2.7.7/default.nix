...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s ffi-yajl-2.7.7 $dest/gems/ffi-yajl-2.7.7-$gp
        ln -s ffi-yajl-2.7.7 $dest/extensions/${arch}/${rubyVersion}/ffi-yajl-2.7.7-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf ffi-yajl-2.7.7 $dest/gems/ffi-yajl-2.7.7-universal-darwin
          ln -sf ffi-yajl-2.7.7 $dest/extensions/${arch}/${rubyVersion}/ffi-yajl-2.7.7-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/ffi-yajl-2.7.7.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/ffi-yajl-2.7.7-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "ffi-yajl"
      s.version = "2.7.7"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/ffi-yajl-2.7.7-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "ffi-yajl"
      s.version = "2.7.7"
    ...
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("ffi-yajl", "ffi-yajl-bench", "2.7.7")
    BINSTUB
