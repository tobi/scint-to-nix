...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/ffi-yajl-3.0.0
        cp -r . $dest/gems/ffi-yajl-3.0.0/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/ffi-yajl-3.0.0
        mkdir -p $extdir
        ...
        ln -s ffi-yajl-3.0.0 $dest/gems/ffi-yajl-3.0.0-$gp
        ln -s ffi-yajl-3.0.0 $dest/extensions/${arch}/${rubyVersion}/ffi-yajl-3.0.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf ffi-yajl-3.0.0 $dest/gems/ffi-yajl-3.0.0-universal-darwin
          ln -sf ffi-yajl-3.0.0 $dest/extensions/${arch}/${rubyVersion}/ffi-yajl-3.0.0-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/ffi-yajl-3.0.0.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/ffi-yajl-3.0.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "ffi-yajl"
      s.version = "3.0.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/ffi-yajl-3.0.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "ffi-yajl"
      s.version = "3.0.0"
    ...
    #!/usr/bin/env ruby
    require "rubygems"
    load Gem.bin_path("ffi-yajl", "ffi-yajl-bench", "3.0.0")
    BINSTUB
