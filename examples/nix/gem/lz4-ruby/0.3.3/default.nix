...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf lz4-ruby-0.3.3 $dest/gems/lz4-ruby-0.3.3-universal-darwin
          ln -sf lz4-ruby-0.3.3 $dest/extensions/${arch}/${rubyVersion}/lz4-ruby-0.3.3-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/lz4-ruby-0.3.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "lz4-ruby"
      s.version = "0.3.3"
