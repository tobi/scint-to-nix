...
let
  ...
  overlay = import ../../../../overlays/field_test.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/field_test-0.6.0
        cp -r . $dest/gems/field_test-0.6.0/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/field_test-0.6.0
        mkdir -p $extdir
        ...
        ln -s field_test-0.6.0 $dest/gems/field_test-0.6.0-$gp
        ln -s field_test-0.6.0 $dest/extensions/${arch}/${rubyVersion}/field_test-0.6.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf field_test-0.6.0 $dest/gems/field_test-0.6.0-universal-darwin
          ln -sf field_test-0.6.0 $dest/extensions/${arch}/${rubyVersion}/field_test-0.6.0-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/field_test-0.6.0.gemspec <<'EOF'
    Gem::Specification.new do |s|
      s.name = "field_test"
      s.version = "0.6.0"
      s.summary = "field_test"
      s.require_paths = ["lib"]
      s.files = []
    end
    EOF
        cat > $dest/specifications/field_test-0.6.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "field_test"
      s.version = "0.6.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/field_test-0.6.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "field_test"
      s.version = "0.6.0"
