...
let
  ...
  overlay = import ../../../../overlays/hiredis-client.nix { inherit pkgs ruby; };
...
in
stdenv.mkDerivation {
  pname = "hiredis-client";
  version = "0.26.4";
  src = builtins.path {
    path = ./source;
    name = "hiredis-client-0.26.4-source";
  };
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/hiredis-client-0.26.4
        cp -r . $dest/gems/hiredis-client-0.26.4/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/hiredis-client-0.26.4
        mkdir -p $extdir
        ...
        ln -s hiredis-client-0.26.4 $dest/gems/hiredis-client-0.26.4-$gp
        ln -s hiredis-client-0.26.4 $dest/extensions/${arch}/${rubyVersion}/hiredis-client-0.26.4-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf hiredis-client-0.26.4 $dest/gems/hiredis-client-0.26.4-universal-darwin
          ln -sf hiredis-client-0.26.4 $dest/extensions/${arch}/${rubyVersion}/hiredis-client-0.26.4-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/hiredis-client-0.26.4.gemspec <<'EOF'
    Gem::Specification.new do |s|
      s.name = "hiredis-client"
      s.version = "0.26.4"
      s.summary = "hiredis-client"
      s.require_paths = ["lib"]
      s.files = []
    end
    EOF
        cat > $dest/specifications/hiredis-client-0.26.4-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "hiredis-client"
      s.version = "0.26.4"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/hiredis-client-0.26.4-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "hiredis-client"
      s.version = "0.26.4"
