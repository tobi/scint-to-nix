...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf bindex-0.8.1 $dest/gems/bindex-0.8.1-universal-darwin
          ln -sf bindex-0.8.1 $dest/extensions/${arch}/${rubyVersion}/bindex-0.8.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/bindex-0.8.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "bindex"
      s.version = "0.8.1"
