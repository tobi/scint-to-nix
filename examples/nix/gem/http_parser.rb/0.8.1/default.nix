...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf http_parser.rb-0.8.1 $dest/gems/http_parser.rb-0.8.1-universal-darwin
          ln -sf http_parser.rb-0.8.1 $dest/extensions/${arch}/${rubyVersion}/http_parser.rb-0.8.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/http_parser.rb-0.8.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "http_parser.rb"
      s.version = "0.8.1"
