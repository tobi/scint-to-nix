...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s erb-6.0.0 $dest/gems/erb-6.0.0-$gp
        ln -s erb-6.0.0 $dest/extensions/${arch}/${rubyVersion}/erb-6.0.0-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf erb-6.0.0 $dest/gems/erb-6.0.0-universal-darwin
          ln -sf erb-6.0.0 $dest/extensions/${arch}/${rubyVersion}/erb-6.0.0-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/erb-6.0.0-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "erb"
      s.version = "6.0.0"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/erb-6.0.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "erb"
      s.version = "6.0.0"
