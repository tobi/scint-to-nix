...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf rbtree-0.4.6 $dest/gems/rbtree-0.4.6-universal-darwin
          ln -sf rbtree-0.4.6 $dest/extensions/${arch}/${rubyVersion}/rbtree-0.4.6-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/rbtree-0.4.6-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "rbtree"
      s.version = "0.4.6"
