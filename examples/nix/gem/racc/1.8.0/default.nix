...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf racc-1.8.0 $dest/gems/racc-1.8.0-universal-darwin
          ln -sf racc-1.8.0 $dest/extensions/${arch}/${rubyVersion}/racc-1.8.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/racc-1.8.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "racc"
      s.version = "1.8.0"
