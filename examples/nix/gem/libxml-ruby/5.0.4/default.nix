...
let
  ...
  overlay = import ../../../../overlays/libxml-ruby.nix { inherit pkgs ruby; };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf libxml-ruby-5.0.4 $dest/gems/libxml-ruby-5.0.4-universal-darwin
          ln -sf libxml-ruby-5.0.4 $dest/extensions/${arch}/${rubyVersion}/libxml-ruby-5.0.4-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/libxml-ruby-5.0.4-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "libxml-ruby"
      s.version = "5.0.4"
