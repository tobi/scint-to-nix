...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf libyajl2-1.2.0 $dest/gems/libyajl2-1.2.0-universal-darwin
          ln -sf libyajl2-1.2.0 $dest/extensions/${arch}/${rubyVersion}/libyajl2-1.2.0-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/libyajl2-1.2.0-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "libyajl2"
      s.version = "1.2.0"
