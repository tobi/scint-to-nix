...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        ln -s byebug-11.1.3 $dest/gems/byebug-11.1.3-$gp
        ln -s byebug-11.1.3 $dest/extensions/${arch}/${rubyVersion}/byebug-11.1.3-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf byebug-11.1.3 $dest/gems/byebug-11.1.3-universal-darwin
          ln -sf byebug-11.1.3 $dest/extensions/${arch}/${rubyVersion}/byebug-11.1.3-universal-darwin
        fi
    ...
    EOF
        cat > $dest/specifications/byebug-11.1.3-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "byebug"
      s.version = "11.1.3"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/byebug-11.1.3-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "byebug"
      s.version = "11.1.3"
