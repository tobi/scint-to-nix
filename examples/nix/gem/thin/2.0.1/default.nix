...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf thin-2.0.1 $dest/gems/thin-2.0.1-universal-darwin
          ln -sf thin-2.0.1 $dest/extensions/${arch}/${rubyVersion}/thin-2.0.1-universal-darwin
        fi
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/thin-2.0.1-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "thin"
      s.version = "2.0.1"
