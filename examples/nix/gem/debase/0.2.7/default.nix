...
stdenv.mkDerivation {
  ...
  installPhase = ''
        ...
        mkdir -p $dest/gems/debase-0.2.7
        cp -r . $dest/gems/debase-0.2.7/
        local extdir=$dest/extensions/${arch}/${rubyVersion}/debase-0.2.7
        mkdir -p $extdir
        ...
        ln -s debase-0.2.7 $dest/gems/debase-0.2.7-$gp
        ln -s debase-0.2.7 $dest/extensions/${arch}/${rubyVersion}/debase-0.2.7-$gp
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          ln -sf debase-0.2.7 $dest/gems/debase-0.2.7-universal-darwin
          ln -sf debase-0.2.7 $dest/extensions/${arch}/${rubyVersion}/debase-0.2.7-universal-darwin
        fi
        mkdir -p $dest/specifications
        cat > $dest/specifications/debase-0.2.7.gemspec <<'EOF'
    ...
    EOF
        cat > $dest/specifications/debase-0.2.7-$gp.gemspec <<PLATSPEC
    Gem::Specification.new do |s|
      s.name = "debase"
      s.version = "0.2.7"
      s.platform = "$gp"
    ...
    PLATSPEC
        if [ "${stdenv.hostPlatform.parsed.kernel.name}" = "darwin" ]; then
          cat > $dest/specifications/debase-0.2.7-universal-darwin.gemspec <<'UNISPEC'
    Gem::Specification.new do |s|
      s.name = "debase"
      s.version = "0.2.7"
