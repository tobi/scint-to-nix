...
if skip || !platformOk then
  ...
  pkgs.runCommand key {} ''
    ...
    ''}
    # Pre-compute SHA-512 manifest (matches real packages)
    f="$out/node_modules/${pkgName}/package.json"
    printf '%s\t%s\t%s\t%s\n' "package.json" \
      "$(sha512sum "$f" | cut -c1-128)" \
      "$(stat -c %a "$f")" \
      "$(stat -c %s "$f")" \
      >> "$out/.onix-manifest"
  ''
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    ${lib.optionalString (integrity != null) ''
      printf '#integrity %s\n' "${integrity}" > "$out/.onix-manifest"
    ''}
    # Pre-compute SHA-512 hashes for each file (cached by Nix, free on rebuild)
    (cd "$out/node_modules/${pkgName}" && find . -type f | sort | while read -r f; do
      rel="''${f#./}"
      hash=$(sha512sum "$f" | cut -c1-128)
      mode=$(stat -c %a "$f")
      size=$(stat -c %s "$f")
      printf '%s\t%s\t%s\t%s\n' "$rel" "$hash" "$mode" "$size"
    done) >> "$out/.onix-manifest"
  '';
