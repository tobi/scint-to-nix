...
#
# Usage:
#   cd ~/src/ruby-tests/solidus && nix-shell ../../tries/2026-02-07-scint/gemset2nix/tests/solidus/devshell.nix
#
{ pkgs ? import <nixpkgs> {}
, ruby ? pkgs.ruby_3_4
}:
...
let
  ...
  gems = resolve { inherit pkgs ruby; gemset = { gem.app.solidus.enable = true; }; };
in pkgs.mkShell {
  ...
  shellHook = ''
    ...
    echo "solidus devshell ready — ${gems.bundlePath}"
